...
*theme* owns rendering, exactly as Magento expects.
...
`make()` accepts a name as its third argument. Re-using a name replaces rather than appends, which is handy
...
Porting "flash messages" to Magento wasn't a copy but a translation: session bag → dispatched
